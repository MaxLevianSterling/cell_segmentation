...
def train(
    ...
    # Send network to CPU
    FusionNet = FusionNet.to(device='cpu', dtype=torch.float)
    
    # Send optimizer to CPU
    ...
    # Wrap model for parallel GPU usage and send to GPU
    ...
    for iE in range(n_epochs):
        ...
        if not (iE+1) % save_snapshot_interval:
            ...
            # Send network to GPU
            FusionNet = FusionNet.to(device=nn_handler_device, dtype=torch.float)
