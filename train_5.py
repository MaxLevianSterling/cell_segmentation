...
def train(
    ...
    # TRY SAVING MODEL AS WELL
    # optimizer and network loading order is opposite according to internet
    ...
    # maybe it's the float() casts?
    # Optional model checkpoint loading
    if load_checkpoint:
        map_location_path = f'{models_folder}FusionNet_map_location{load_checkpoint}.tar'
        map_location = torch.load(map_location_path)

        checkpoint_path = f'{models_folder}FusionNet_checkpoint{load_checkpoint}.tar'
        checkpoint = torch.load(checkpoint_path, map_location=map_location['map_location'])
        # CHECK IF MODEL UPDATES WHILE RETURNING model_loading
        FusionNet = torch.load(checkpoint['model'], map_location=map_location['map_location'])
        model_loading = FusionNet.module.load_state_dict(checkpoint['model_module_state_dict'])
    else:
    ...
    if load_checkpoint:
        if nn_handler_device != map_location['map_location']:
            raise ValueError

    # Define optimizer and send to GPU
    # CHECK if optimizer lr is already under 2
    optimizer = torch.optim.Adam(FusionNet.parameters(), lr=lr, weight_decay=weight_decay)
    #!!!# TRY only sending it to GPU when not load_checkpoint
    # Also try not sending it at all
    optimizer_to(optimizer, torch.device(nn_handler_device))

    if load_checkpoint:
        optimizer_loading = optimizer.load_state_dict(checkpoint['optimizer_state_dict'])

    # Define scheduler
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=gamma)

    if load_checkpoint:
        scheduler_loading = scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
    ...
    # TRY WRAPPING MODEL BEFORE OPTIMIZER LOADING
    # FusionNet = nn.DataParallel(
    #     FusionNet,
    #     device_ids=gpu_device_ids
    # )
    ...
    for iE in range(n_epochs):
        ...
        for iB, batch in enumerate(dataloader):
            ...
            if amp:
                with torch.cuda.amp.autocast():
                    ...
                    y_ = Variable(batch['annot']).to(device=nn_handler_device, dtype=torch.uint8)
                    y = FusionNet(x)
                    loss = loss_func(y, y_)
            else:
                x = Variable(batch['image']).to(device=nn_handler_device, dtype=torch.float)
                ...
                #     f'/mnt/sdg/maxs/results/LIVECell/FusionNet_orient_x_checkpoint_image.png'
                # )
                # v_utils.save_image(
                #     y_[0,0:1,:,:].detach().to('cpu').type(torch.float32),
                #     f'/mnt/sdg/maxs/results/LIVECell/FusionNet_orient_y__checkpoint_image.png'
                # )
                # v_utils.save_image(
                #     y[0,0:1,:,:].detach().to('cpu').type(torch.float32),
                #     f'/mnt/sdg/maxs/results/LIVECell/FusionNet_orient_y_checkpoint_image.png'
                # )
        ...
        if not (iE+1) % save_checkpoint_interval:
            ...
            torch.save({
                'model': copy.deepcopy(FusionNet),
                'model_state_dict': copy.deepcopy(FusionNet.state_dict()),
                'model_module_state_dict': copy.deepcopy(FusionNet.module.state_dict()),
            ...
            # Save FusionNet map location
            torch.save({
                'map_location': nn_handler_device
            }, f'{models_folder}FusionNet_map_location{load_checkpoint+iE+1}.tar')
